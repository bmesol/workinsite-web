...
//     // PIN validation
//     if (!pin || pin.length === 0) updateError("pin", "Please enter pin");
//     else if (pin.length !== 4) updateError("pin", "Pin must be 4 digits");

//     // Confirm PIN validation (only if provided)
...
const useInputValidate = (props: InputValidateProps) => {
  ...
  const validate = () => {
   ...
   // Name validation
if (name !== undefined) {
  const trimmedName = name.trim().replace(/\s+/g, " "); // ✅ multiple spaces → single space
  if (trimmedName.length === 0) 
    ...
    if (phoneNumber !== undefined) { // ✅ conditional panninom
      if (!phoneNumber || phoneNumber.length === 0)
        updateError("phoneNumber", "Please enter phone number");
      else if (phoneNumber.length !== 10)
        updateError("phoneNumber", "Phone number must be 10 digits");
    ...
    // PIN validation (only if provided) ✅
    if (pin !== undefined) {
