...
import { getTaskFieldPermissions } from '@/shared/helpers/TaskPermission';
import { seedSelfUpdate } from '@/shared/features/notifications/hooks/useTaskNotifications';
...
export const useTaskEdit = (id: string) => {
  ...
  const fetchTask = async () => {
    try {
      ...
      const supervisors = data.site?.supervisors || [];

      setSupervisorList(supervisors);
      setTaskName(data.taskName);
      setSiteId(String(data.site?.id));
      setSiteList([data.site]);
      // Task DTO has assignedTo/assignedBy, not a flat "supervisor" field.
      setSupervisorId(data.assignedTo?.id ? String(data.assignedTo.id) : '');
      setDate(data.date);
      setPriority(data.priority);
      setStatus(data.status);
      setRemarks(data.remarks || []);
  ...
  const handleSiteChange = async (newSiteId: string) => {
    ...
    try {
      const site = await siteService.getSite(parseInt(newSiteId));
      // NOTE: mobile does not filter this by role name — everyone linked
      // to the site under `supervisors` is eligible for assignment.
  ...
  // ── Assignment-aware permissions (ported from mobile) ────────────────────
  const isAssignee =
    user?.id !== undefined &&
    String(user.id) === String(taskDetails?.assignedTo?.id ?? '');

  const taskFieldPermissions = useMemo(
    () =>
      getTaskFieldPermissions({
        loggedInRoleId: user?.role?.id,
        assignedByRoleId: taskDetails?.assignedBy?.role?.id,
        hasBaseEditPermission: editable,
      }),
    [user?.role?.id, taskDetails?.assignedBy?.role?.id, editable],
  );
  ...
  const hasUnsavedChanges = useCallback(() => {
    ...
    return (
      taskName !== taskDetails?.taskName ||
      siteId !== taskDetails?.site?.id?.toString() ||
      supervisorId !== (taskDetails?.assignedTo?.id?.toString() ?? '') ||
      date !== taskDetails?.date ||
      priority !== taskDetails?.priority ||
      status !== taskDetails?.status ||
      newRemark !== '' ||
  ...
  const handleSubmission = async () => {
    try {
      ...
      const form = new FormData();
      form.append('SiteId', siteId);
      form.append('AssignedToId', supervisorId);
      form.append('TaskName', taskName.trim());
      form.append('Date', date.trim());
      form.append(
        'Priority',
      ...
      // Seed our own updatedOn tracker so the notification poller doesn't
      // fire a self-notification for the change we just submitted.
      try {
        const fresh = await taskService.getTask(parseInt(id));
